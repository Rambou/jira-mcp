...
A Model Context Protocol (MCP) server for common Jira REST API actions, with optional Tempo worklog support.
...
- `TEMPO_API_TOKEN` (optional): Tempo API token. When set, Tempo worklog tools are enabled.
- `TEMPO_BASE_URL` (optional): Tempo API base URL, default `https://api.tempo.io/4`
...
These tools are only available when `TEMPO_API_TOKEN` is set.
...
> **Note:** The Tempo worklog tools use the current Jira user (identified by `JIRA_TOKEN`) as the worklog author. The `tempo_list_worklogs` tool also uses that user to scope the returned worklogs.
